...
- lane 0과 lane 1의 가로 간격은 최소 40px이다. 이 경계를 통과하는 edge가 여러 개면 첫 edge 이후 edge마다 10px 트랙 여유를 추가해 확장한다. lane 1 이후의 열 간격도 연결선 수에 따라 확장한다. 직선 트리 edge 이외의 연결선이 많은 레벨 간격도 세로로 넓힌다.
