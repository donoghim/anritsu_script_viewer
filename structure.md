...
- 보조 lane 우선순위는 주 흐름에서 직접 분기된 노드, 그 외 incoming edge가 있는 노드, 독립 진입 노드 순서다. 같은 레벨에서는 이 순서로 lane을 배정하며, lane 재정렬과 표시 열 재사용에서도 유지한다. 따라서 독립 진입 노드는 주 흐름의 직접 분기를 두 번째 열에서 밀어내지 않는다.
